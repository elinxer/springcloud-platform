...
import com.elinxer.springcloud.platform.mqtt.broker.constants.ShareStrategy;
...
import com.elinxer.springcloud.platform.mqtt.broker.entity.PubMsg;
...
import com.elinxer.springcloud.platform.mqtt.broker.service.*;
import com.elinxer.springcloud.platform.mqtt.broker.utils.JsonSerializer;
import com.elinxer.springcloud.platform.mqtt.broker.utils.RateLimiter;
...
@Slf4j
public class
PublishHandler extends AbstractMqttTopicSecureHandler implements Watcher {
    //@formatter:off
    ...
    public PublishHandler(IPublishMessageService publishMessageService, IRetainMessageService retainMessageService,
        ...
        if (ShareStrategy.round == shareStrategy) {
        ...
        executor = new ThreadPoolExecutor(coreSize, coreSize * 2,
    ...
    public void process(ChannelHandlerContext ctx, MqttMessage msg) {
        ...
        //是否需要回復 測試使用 start
        if (iotMessageDelegatingProcess != null) {
            executor.execute(() -> {
                iotMessageDelegatingProcess.process(data, ctx);
            });
        }
        //是否需要回復 測試使用 end
    }
