...
package com.elinxer.springcloud.platform.mqtt.broker.broker;

import com.alibaba.fastjson.TypeReference;
import com.elinxer.springcloud.platform.mqtt.broker.broker.handler.AbstractMqttSessionHandler;
import com.elinxer.springcloud.platform.mqtt.broker.broker.handler.ConnectHandler;
import com.elinxer.springcloud.platform.mqtt.broker.broker.handler.MessageDelegatingHandler;
import com.elinxer.springcloud.platform.mqtt.broker.broker.handler.PublishHandler;
...
import com.elinxer.springcloud.platform.mqtt.broker.exception.AuthenticationException;
import com.elinxer.springcloud.platform.mqtt.broker.exception.AuthorizationException;
import com.elinxer.springcloud.platform.mqtt.broker.iotdevice.IotMessageDelegatingProcessor;
import com.elinxer.springcloud.platform.mqtt.broker.iotdevice.constants.ProtocConstants;
import com.elinxer.springcloud.platform.mqtt.broker.service.ISessionService;
...
public class BrokerHandler extends SimpleChannelInboundHandler<MqttMessage> implements Watcher {
    ...
    public void channelInactive(ChannelHandlerContext ctx) {
        // 获取当前会话
        Object obj = ctx.channel().attr(AttributeKey.valueOf(Session.KEY)).get();
        Session session = null;
        if (obj != null) {
            session = (Session) obj;
            log.info("channelInactive-clientId={}", session.getClientId());
        }
    ...
    private void onClientDisconnected(ChannelHandlerContext ctx, Session session) {
        ...
        Optional.of(session)
                ...
                .ifPresent(msg -> {
                    ...
                    if (session.getLoginStatus() == ProtocConstants.LOGIN_REQ) {
                        log.info("clientId={},disconnect", session.getClientId());
                        iotMessageDelegatingProcess.process(msg.getPayload(), ctx);
                    }
                });
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
        ...
        if (cause instanceof MqttIdentifierRejectedException) {
            ...
            log.error("未知异常 ip={},ex={}", ctx.channel().remoteAddress(), cause);
        }
    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) {
        ...
        if (evt instanceof IdleStateEvent) {
            if (IdleState.ALL_IDLE.equals(((IdleStateEvent) evt).state())) {
                Session session = (Session) ctx.channel().attr(AttributeKey.valueOf(Session.KEY)).get();
                log.info("R[{}:{}] 心跳超时,session={}", host, port, session);
                // 关闭连接
                ctx.close();
            }
            ...
            if (!((SslHandshakeCompletionEvent) evt).isSuccess()) {
                ...
                log.warn("R[{}:{}] ssl 握手失败,{}", host, port, event.cause());
                ctx.close();
            }
        }
    }
