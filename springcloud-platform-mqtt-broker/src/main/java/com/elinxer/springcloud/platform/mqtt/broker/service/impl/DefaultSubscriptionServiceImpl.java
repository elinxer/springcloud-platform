...
import com.elinxer.springcloud.platform.mqtt.broker.entity.ClientSubOrUnsubMsg;
...
import com.elinxer.springcloud.platform.mqtt.broker.utils.TopicUtils;
...
public class DefaultSubscriptionServiceImpl implements ISubscriptionService, Watcher {
    ...
    public void subscribe(ClientSub clientSub) {
        ...
        boolean cleanSession = clientSub.isCleanSession();
         log.info("enableTestMode={},ClientSub={}",enableTestMode, clientSub);
        if (enableTestMode) {
            ...
            if (cleanSession) {
                ...
                inMemClientTopicsMap
                        ...
                        .add(topic);
                log.info("cache-inMemTopicClientsMap={}",JSONObject.toJSONString(inMemTopicClientsMap));
                log.info("cache-inMemTopics={}",JSONObject.toJSONString(inMemTopics));
                log.info("cache-inMemClientTopicsMap={}",JSONObject.toJSONString(inMemClientTopicsMap));
            } else {
                ...
                log.info("cleanSession={},clientId={},toppic={}",cleanSession,clientId,topic);
            }
    ...
    public List<ClientSub> searchSubscribeClientList(String topic) {
        // 启用内部缓存机制
        log.info("1111111111-topic={},enableInnerCache={},enableTestMode={}",topic,enableInnerCache,enableTestMode);
        if (enableInnerCache || enableTestMode) {
        ...
        log.info("1111111111-topicSetKey={},redisTopics={}",topicSetKey,redisTopics);
        ...
        redisTopics.stream()
                ...
                .forEach(e -> {
                    ...
                    log.info("1111111111-hashkey={},entries={}",(topicPrefix + e),JSONObject.toJSONString(entries));
        ...
        inMemTopics.stream()
                ...
                .forEach(e -> {
                    ConcurrentHashMap.KeySetView<ClientSub, Boolean> clientSubs = inMemTopicClientsMap.get(topic);
                    log.info("1111111111-topic={},clientSubs={}",topic,JSONObject.toJSONString(clientSubs));
                    if (!CollectionUtils.isEmpty(clientSubs)) {
    ...
    private List<ClientSub> searchSubscribeClientListByCache(String topic) {
        // result
        List<ClientSub> clientSubList = new ArrayList<>();
        log.info("22222222222-inDiskTopics={}",JSONObject.toJSONString(inDiskTopics));
        ...
        log.info("22222222222-inMemTopics={}",JSONObject.toJSONString(inMemTopics));
        for (String t : inMemTopics) {
            if(t==null){
                continue;
            }
            String a1 =t.replaceAll("/","");
            String b2 =topic.replaceAll("/","");
            log.info("a1={},b2={},match={}",a1,b2,a1.equals(b2));
            Boolean match=TopicUtils.match(topic, t);
            log.info("topicB={},topicL={}",topic.getBytes(StandardCharsets.UTF_8),topic.length());
            log.info("tB2222={},tL2222={}",t.getBytes(StandardCharsets.UTF_8),t.length());
            log.info("22222222222-t={},topic={},match={}",t,topic,match);
            if (match) {
                log.info("22222222222-inMemTopicClientsMap={}",JSONObject.toJSONString(inMemClientTopicsMap));
                ConcurrentHashMap.KeySetView<ClientSub, Boolean> clientSubs = inMemTopicClientsMap.get(t);
                if (!CollectionUtils.isEmpty(clientSubs)) {
                    clientSubList.addAll(clientSubs);
                }
    ...
    public static void main(String[] args) {
        Set<String> inMemTopics = ConcurrentHashMap.newKeySet(ASSUME_COUNT);

        String pub="vehicle/%s/tbox";
        String pub0="vehicle/861617053336771/tbox";
        String  sub= "vehicle/861617053336771/tbox";
        inMemTopics.add(pub0);
        for(String t:inMemTopics){
            System.out.println(Objects.equals(t,sub));
            System.out.println(TopicUtils.match(t,sub));
        }
        System.out.println(Objects.equals(pub0,sub));
